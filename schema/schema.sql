...
CREATE TABLE events (
    ...
    search_tsv  TSVECTOR GENERATED ALWAYS AS (
        CASE WHEN kind IN (1059, 30300, 30350, 30622, 44100, 44101, 44200, 44210, 44211, 44212) THEN NULL::tsvector
...
-- Paid runtime reservations are claimed transactionally with instruction
-- insertion. This row is relay coordination state; signed 44211/44212 events
-- remain the auditable ledger.
CREATE TABLE agent_runtime_reservation_claims (
    ...
    CONSTRAINT chk_runtime_claim_reservation_id_len CHECK (length(reservation_event_id) = 32),
...
BEGIN
    IF NEW.kind = 44211 THEN
        SELECT count(*), min(value->>1) INTO tag_count, payer_hex
        ...
        IF tag_count <> 1 OR payer_hex !~ '^[0-9a-f]{64}$' THEN
            RAISE EXCEPTION 'agent_runtime_claim: reservation requires one canonical p tag'
                USING ERRCODE = '23514';
        END IF;
        SELECT count(*), min(value->>1) INTO tag_count, channel_text
          FROM jsonb_array_elements(NEW.tags) value
         WHERE jsonb_typeof(value) = 'array' AND jsonb_array_length(value) = 2
           AND value->>0 = 'h';
        IF tag_count <> 1 OR channel_text !~
            ...
            RAISE EXCEPTION 'agent_runtime_claim: reservation channel is invalid'
                USING ERRCODE = '23514';
        END IF;
        SELECT count(*), min(value->>1) INTO tag_count, expiration_text
          FROM jsonb_array_elements(NEW.tags) value
         WHERE jsonb_typeof(value) = 'array' AND jsonb_array_length(value) = 2
    ...
    IF NEW.kind = 44212 THEN
        SELECT count(*), min(value->>1) INTO tag_count, reservation_hex
          FROM jsonb_array_elements(NEW.tags) value
         WHERE jsonb_typeof(value) = 'array' AND jsonb_array_length(value) = 2
           AND value->>0 = 'e';
    ...
    FOR tag IN SELECT value FROM jsonb_array_elements(NEW.tags) value
    ...
    LOOP
        ...
        END IF;
        UPDATE agent_runtime_reservation_claims
           SET instruction_event_id = NEW.id, claimed_at = COALESCE(claimed_at, NOW())
         WHERE community_id = NEW.community_id
           AND reservation_event_id = decode(reservation_hex, 'hex')
