...
proof, amount, signer, recipient, target, and channel bindings. Clients can
render and count stored zaps in the same event path as reactions.
...
`placeholder` in the kind `9736` `proof` tag after the payment settles. The
relay treats that exact temporary marker as a valid proof. It also accepts a
payer proof that rust-lightning can parse and cryptographically verify. The
temporary marker does not provide
...
event-ID deduplication at the local history boundary. The relay parses real
`lnp` proofs with rust-lightning. This validates their TLV structure, payment
preimage, invoice signature, and payer signature. During the temporary
compatibility period, the exact `placeholder` marker is valid for the same UI
surfaces and totals.
