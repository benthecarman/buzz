...
test("sends bitcoin without relying on kind-0 metadata", async ({ page }) => {
  ...
  const messageAction = page.getByTestId("user-profile-message");
  const bitcoinAction = page.getByTestId("user-profile-send-bitcoin");
  await expect(bitcoinAction).toContainText("Send bitcoin");
  const [messageActionBox, bitcoinActionBox] = await Promise.all([
    messageAction.boundingBox(),
    bitcoinAction.boundingBox(),
  ]);
  if (!messageActionBox || !bitcoinActionBox) {
    throw new Error("Profile actions must be visible");
  }
  expect(bitcoinActionBox.width).toBeCloseTo(messageActionBox.width, 0);
  expect(bitcoinActionBox.height).toBeCloseTo(messageActionBox.height, 0);
...
test("message zap sends ₿50 optimistically without progress chrome", async ({
  ...
  await expect(optimisticZap).toHaveClass(/\bcursor-pointer\b/);
  await expect(optimisticZap).not.toHaveClass(/\bcursor-help\b/);
