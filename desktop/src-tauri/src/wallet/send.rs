...
use super::{
    conformance,
    models::{WalletError, WalletPaymentResult, WalletPaymentStatus, WalletSendRequest},
};
...
impl SendAttemptStore {
    ...
    fn stored_attempts(&self) -> Result<Vec<SendAttempt>, WalletError> {
        ...
        for entry in entries {
            let entry = match entry {
                Ok(entry) => entry,
                Err(error) => {
                    tracing::warn!(error = %error, "skip unreadable send attempt entry");
                    continue;
                }
            };
            let path = entry.path();
            ...
            let Some(key) = path.file_stem().and_then(|value| value.to_str()) else {
                tracing::warn!(path = %path.display(), "skip unnamed send attempt");
                continue;
            };
            match self.load(key) {
    ...
    fn save(&self, attempt: &mut SendAttempt) -> Result<(), WalletError> {
        ...
        let mut file = super::private_atomic_file(self.path(&attempt.request.request_id)?)
    ...
    /// Return all attempts that may have reached the provider but are not final.
    pub fn paying_attempts(&self) -> Result<Vec<SendAttempt>, WalletError> {
        let mut attempts = self
            .stored_attempts()?
            .into_iter()
            .filter(|attempt| attempt.state == SendAttemptState::Paying)
            .collect::<Vec<_>>();
    ...
    pub fn prune(&self) -> Result<(), WalletError> {
        ...
        for attempt in self.stored_attempts()? {
            if attempt.state.is_terminal() && attempt.updated_at_ms < cutoff {
                let path = self.path(&attempt.request.request_id)?;
...
mod tests {
    ...
    #[test]
    fn send_attempt_scan_keeps_valid_entries_after_corruption() {
        let temp = tempfile::tempdir().unwrap();
        let store = SendAttemptStore::new(temp.path(), &"a".repeat(64));
        let mut attempt = SendAttempt::prepare(WalletSendRequest {
            destination: VALID_OFFER.to_string(),
    ...
    #[test]
    fn paying_attempts_exclude_prepared_and_terminal_sends() {
        let temp = tempfile::tempdir().unwrap();
        let store = SendAttemptStore::new(temp.path(), &"a".repeat(64));
        let request = || WalletSendRequest {
            destination: VALID_OFFER.to_string(),
            amount: Some(21),
            message: None,
            request_id: Uuid::new_v4().to_string(),
        };
        let mut prepared = SendAttempt::prepare(request());
        store.save_prepared(&mut prepared).unwrap();
        let mut paying = SendAttempt::prepare(request());
        store.save_prepared(&mut paying).unwrap();
        store.begin_dispatch(&mut paying).unwrap();
        let mut completed = SendAttempt::prepare(request());
        store.save_prepared(&mut completed).unwrap();
        store.begin_dispatch(&mut completed).unwrap();
        store
            .record_payment(
                &mut completed,
                WalletPaymentResult {
                    ...
                    fees: 0,
                    created_at_ms: 0,
                    finalized_at_ms: Some(1),
                },
            )
            .unwrap();

        assert_eq!(store.paying_attempts().unwrap(), vec![paying]);
    }
