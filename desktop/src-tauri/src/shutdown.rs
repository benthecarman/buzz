...
/// Run the built application and apply the common exit/restart cleanup path.
pub(crate) fn run(app: tauri::App) {
    ...
    #[cfg(unix)]
    install_signal_handler(app.handle().clone(), Arc::clone(&shutdown_done));

    let run_shutdown_done = Arc::clone(&shutdown_done);
    let restart_requested = Arc::new(AtomicBool::new(false));
    app.run(move |app_handle, event| match event {
        #[cfg(target_os = "macos")]
        RunEvent::Reopen { .. } => crate::tray_menu::show_main_window(app_handle),
        #[cfg(target_os = "macos")]
            ...
            let is_active_huddle_window =
                app_handle
                    ...
                    .is_some_and(|huddle| {
                        !matches!(
                            huddle.phase,
                            crate::huddle::HuddlePhase::Idle | crate::huddle::HuddlePhase::Leaving
                        ) && huddle
